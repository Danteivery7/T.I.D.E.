...
function platformTokens(game){return(game?.meta?.platforms||[]).flatMap(p=>[p?.slug,p?.name]).filter(Boolean).map(x=>String(x).toLowerCase());}
function environmentFor(game){
  ...
  const endpoint=String(game?.meta?.endpoint_awards||game?.endpoint_awards||'').toLowerCase(),canonical=String(game?.meta?.canonical_url||game?.canonical_url||'').toLowerCase(),tokens=platformTokens(game),has=x=>tokens.some(t=>t.includes(x)),haystack=`${endpoint} ${canonical}`;
...
async function resolveExophaseId(env){
  ...
  if(!m)throw new Error('player id was not exposed by the profile page');return m[1];
}
...
function addSteam(map,row,source='steam-page'){
  ...
  else{if(lastPlayed&&lastPlayed>String(existing.lastPlayed||''))existing.lastPlayed=lastPlayed;if(!existing.firstPlayed&&firstPlayed)existing.firstPlayed=firstPlayed;if(recentMinutes>Number(existing.recentMinutes||0)){existing.recentMinutes=recentMinutes;existing.hoursLast2Weeks=recentMinutes/60;}map.set(key,existing);}
}
async function steamOfficial(env,map){
  const key=String(env?.STEAM_API_KEY||'').trim();if(!key)return false;
  const u=new URL('https://api.steampowered.com/IPlayerService/GetOwnedGames/v0001/');u.searchParams.set('key',key);u.searchParams.set('steamid',STEAM_ID);u.searchParams.set('include_appinfo','1');u.searchParams.set('include_played_free_games','1');u.searchParams.set('format','json');
  ...
  const rows=(await r.json())?.response?.games;if(!Array.isArray(rows)||!rows.length)throw new Error('Steam Web API returned no owned games');for(const row of rows)addSteam(map,row,'steam-web-api');return true;
}
...
function parseSteamHtml(html,map){const renderText=parseJsonStringLiteral(html,'window.SSR.renderContext=JSON.parse');if(renderText)try{collectSteam(JSON.parse(renderText),map);}catch{}for(const marker of ['window.SSR.loaderData =','window.SSR.loaderData=','var rgGames =','var rgGames=']){const raw=balancedJson(html,marker);if(raw)try{collectSteam(JSON.parse(raw),map);}catch{}}const direct=html.match(/var\s+rgGames\s*=\s*(\[[\s\S]*?\])\s*;/);if(direct)try{collectSteam(JSON.parse(direct[1]),map);}catch{}}
function parseSteamXml(xml,map){for(const m of xml.matchAll(/<game>([\s\S]*?)<\/game>/gi)){const block=m[1],appId=tag(block,'appID'),title=tag(block,'name'),hrs=Number(tag(block,'hoursOnRecord').replace(/,/g,'')||0),recent=Number(tag(block,'hoursLast2Weeks').replace(/,/g,'')||0);if(!title||!appId)continue;addSteam(map,{appid:Number(appId),name:title,playtime_forever:Math.max(0,Math.round(hrs*60)),playtime_2weeks:Math.max(0,Math.round(recent*60)),logo:tag(block,'logo')},'steam-xml');}}
async function steam(env){
  const map=new Map(),methods=[],errors=[];try{if(await steamOfficial(env,map))methods.push('web-api');}catch(error){errors.push(String(error?.message||error));}
  const headers={'user-agent':UA,'accept':'text/html,application/xhtml+xml,application/xml;q=0.8','accept-language':'en-US,en;q=0.9'},urls=[`https://steamcommunity.com/profiles/${STEAM_ID}/games?tab=all&xml=1`,`https://steamcommunity.com/profiles/${STEAM_ID}/games?xml=1`,`https://steamcommunity.com/profiles/${STEAM_ID}/games?tab=all`],responses=await Promise.allSettled(urls.map(u=>fetch(u,{headers})));
  for(let i=0;i<responses.length;i++){const x=responses[i];if(x.status!=='fulfilled'||!x.value.ok)continue;const text=await x.value.text();if(i<2){parseSteamXml(text,map);methods.push(i===0?'xml-all':'xml');}else{parseSteamHtml(text,map);methods.push('community-page');}}
  const games=[...map.values()];if(!games.length)return{ok:false,error:errors[0]||'Steam public sources returned no owned-game playtime',games:[],method:methods.join('+')};return{ok:true,steamId:STEAM_ID,games,method:[...new Set(methods)].join('+')||'community',warnings:errors};
}
function mergeExophaseSteam(stm,exo){
  if(!exo?.ok)return stm;const exoSteam=(exo.games||[]).filter(x=>x.environment==='steam'||x.platform==='steam'),byTitle=new Map((stm.games||[]).map((x,i)=>[titleKey(x.title),i]));
  for(const x of exoSteam){const key=titleKey(x.title);if(!key)continue;const i=byTitle.get(key);if(i!=null){const cur=stm.games[i];if((Number(cur.minutes)||0)<=0&&(Number(x.minutes)||0)>0)cur.minutes=Number(x.minutes)||0;if(x.lastPlayed>String(cur.lastPlayed||''))cur.lastPlayed=x.lastPlayed;if(!cur.firstPlayed&&x.firstPlayed)cur.firstPlayed=x.firstPlayed;if(!cur.image&&x.image)cur.image=x.image;continue;}if((Number(x.minutes)||0)<=0)continue;byTitle.set(key,stm.games.length);stm.games.push({sourceKey:`steam:exophase:${x.masterPlayerId||'player'}:${x.masterId||key}`,appId:null,title:x.title,minutes:Number(x.minutes)||0,recentMinutes:0,hoursLast2Weeks:0,firstPlayed:x.firstPlayed||'',lastPlayed:x.lastPlayed||'',platform:'steam',environment:'steam',image:x.image||'',source:'exophase-steam-fallback'});}
  if(!stm.ok&&stm.games.length){stm.ok=true;stm.error='';stm.method='exophase-steam-fallback';}return stm;
}
