...
function process(data){
  ...
  const previous=new Map((g.latestSources||[]).map(x=>[x.sourceKey,x])),next=[],exoSteam=[];let ubiIdentity=null,keptUbisoft=[];
  if(data.exophase?.ok){
  ...
  if(data.steam?.ok){
    for(const x of data.steam.games||[]){const k=keyTitle(x.title);steamAdded.add(k);next.push({...x,sourceFamily:'steam',platform:'steam',countMinutes:true,firstPlayed:x.firstPlayed||g._steamFirst?.[k]||'',lastPlayed:maxDate(x.lastPlayed,g._steamLast?.[k])});}
  }
  ...
  if(!first){
    for(const cur of next){const old=previous.get(cur.sourceKey);if(!old)continue;const delta=(Number(cur.minutes)||0)-(Number(old.minutes)||0);if(delta<=0)continue;const k=keyTitle(cur.title),item=deltaGames[k]||{key:k,title:displayTitle(cur.title),platformMinutes:{},lastPlayed:'',image:cur.image||''};item.platformMinutes[cur.platform]=(item.platformMinutes[cur.platform]||0)+delta;item.lastPlayed=maxDate(item.lastPlayed,cur.lastPlayed);deltaGames[k]=item;}
  }
  ...
  g.latestSources=next;g.lastRefreshAt=data.capturedAt||now();g.lastRefreshLocalDate=date;g.liveSourceVersion=PC_SOURCE_VERSION;g.liveStatus={exophase:data.exophase||{},steam:data.steam||{}};saveGaming(g);
